...
func (i *orderDatasource) CreateOrder(tx *gorm.DB, data *entity.Order) (*entity.Order, error) {
	...
	var res entity.Order
	result := tx.Raw(`INSERT INTO "order" ("id", "business_thumbnail", "status", "business_name", "items_quantity", "authorization_token_id", "business_id", "coordinates", "start_order_time", "end_order_time", "order_type", "number", "address", "instructions", "price_cup", "user_id", "create_time", "update_time", "short_id", "payment_method_type", "phone") VALUES (?, ?, ?, ?, ?, ?, ?, ST_GeomFromText(?, 4326), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING "id", "business_thumbnail", "short_id", "business_name", "items_quantity", "status", "order_type", "price_cup", "number", "address", "business_id", ST_AsEWKB(coordinates) AS coordinates, "user_id", "authorization_token_id", "start_order_time", "end_order_time", "instructions", "cancel_reasons", "create_time", "update_time", "payment_method_type", "phone"`, uuid.New().String(), data.BusinessThumbnail, data.Status, data.BusinessName, data.ItemsQuantity, data.AuthorizationTokenId, data.BusinessId, point, data.StartOrderTime, data.EndOrderTime, data.OrderType, data.Number, data.Address, data.Instructions, data.PriceCup, data.UserId, time, time, shortId, data.PaymentMethodType, data.Phone).Scan(&res)
	if result.Error != nil {
		return nil, result.Error
	}
	return &res, nil
