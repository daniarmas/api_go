...
func (i *businessScheduleRepository) BusinessIsOpen(tx *gorm.DB, where *entity.BusinessSchedule) (bool, error) {
	result, err := Datasource.NewBusinessScheduleDatasource().BusinessIsOpen(tx, where)
	if err != nil {
		return false, err
	}
	return result, nil
