...
func (i *municipalityDatasource) MunicipalityByCoordinate(tx *gorm.DB, coordinate ewkb.Point) (*models.Municipality, error) {
	...
	result := tx.Select("id, name, province_fk, ST_AsEWKB(coordinates) AS coordinates, zoom, create_time, update_time").Where("ST_Contains(polygon, ST_GeomFromText(?, 4326))", p).Take(&municipalityResult)
	if result.Error != nil {
