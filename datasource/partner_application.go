...
func (i *partnerApplicationDatasource) GetPartnerApplication(tx *gorm.DB, where *models.PartnerApplication) (*models.PartnerApplication, error) {
	var res models.PartnerApplication
	result := tx.Raw(`SELECT "id", "business_name", "description", "status", ST_AsEWKB(coordinates) AS coordinates, "province_id", "municipality_id", "create_time", "update_time" FROM "partner_application" WHERE id = ? LIMIT 1`, where.ID).Scan(&res)
	if result.Error != nil {
...
func (i *partnerApplicationDatasource) UpdatePartnerApplication(tx *gorm.DB, where *models.PartnerApplication, data *models.PartnerApplication) (*models.PartnerApplication, error) {
	...
	result := tx.Raw(`UPDATE "status"=?,"update_time"=? WHERE "partner_application"."id" = ? AND "partner_application"."delete_time" IS NULL RETURNING "id", "business_name", "description", "status", ST_AsEWKB(coordinates) AS coordinates, "province_id", "municipality_id", "create_time", "update_time"`, data.Status, time, where.ID).Scan(&res)
	if result.Error != nil {
		if result.Error.Error() == "record not found" {
			return nil, errors.New("record not found")
		} else {
