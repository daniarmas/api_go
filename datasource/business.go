...
func (b *businessDatasource) CreateBusiness(tx *gorm.DB, data *models.Business) (*models.Business, error) {
	...
	result := tx.Raw(`INSERT INTO "business" ("id", "name", "address", "high_quality_photo", "low_quality_photo", "thumbnail", "blurhash", "time_margin_order_month", "time_margin_order_day", "time_margin_order_hour", "time_margin_order_minute", "delivery_price", "to_pick_up", "home_delivery", "coordinates", "province_id", "municipality_id", "business_brand_id",  "create_time", "update_time", "cursor") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ST_GeomFromText(?, 4326), ?, ?, ?, ?, ?, ?) RETURNING "id", "name", "description", "address", "high_quality_photo", "low_quality_photo", "thumbnail", "blurhash", "time_margin_order_month", "time_margin_order_day", "time_margin_order_hour", "time_margin_order_minute", "delivery_price", "to_pick_up", "home_delivery", ST_AsEWKB(coordinates) AS coordinates, "province_id", "municipality_id", "business_brand_id",  "create_time", "update_time", "cursor"`, uuid.New().String(), data.Name, data.Address, data.HighQualityPhoto, data.LowQualityPhoto, data.Thumbnail, data.BlurHash, data.TimeMarginOrderMonth, data.TimeMarginOrderDay, data.TimeMarginOrderHour, data.TimeMarginOrderMinute, data.DeliveryPrice, data.ToPickUp, data.HomeDelivery, point, data.ProvinceId, data.MunicipalityId, data.BusinessBrandId, time, time, number.RowsAffected+1).Scan(&res)
	if result.Error != nil {
		return nil, result.Error
	}
	return &res, nil
...
func (b *businessDatasource) Feed(tx *gorm.DB, coordinates ewkb.Point, limit int32, provinceId string, municipalityId string, cursor int32, municipalityNotEqual bool, homeDelivery bool, toPickUp bool) (*[]models.Business, error) {
	...
	query := fmt.Sprintf("SELECT id, name, address, high_quality_photo, low_quality_photo, blurhash, delivery_price, home_delivery, to_pick_up, business_brand_id, province_id, municipality_id, cursor FROM business %v ORDER BY cursor asc LIMIT 6;", where)
...
func (b *businessDatasource) GetBusinessWithDistance(tx *gorm.DB, where *models.Business) (*models.Business, error) {
	var businessResult *models.Business
	...
	query := fmt.Sprintf("SELECT business.id, business.name, business.business_brand_id, business.municipality_id, business.province_id, business.thumbnail, business.blurhash, business.address, business.high_quality_photo, business.time_margin_order_month, business.time_margin_order_day, business.time_margin_order_hour, business.time_margin_order_minute, business.low_quality_photo, business.delivery_price, business.home_delivery, business.to_pick_up, business.cursor, ST_AsEWKB(business.coordinates) AS coordinates, %v FROM business WHERE business.id = '%v' ORDER BY business.cursor asc LIMIT 1;", distance, where.ID)
	err := tx.Raw(query).Scan(&businessResult).Error
	if err != nil {
		return nil, err
