...
func (b *businessDatasource) GetBusinessWithDistance(tx *gorm.DB, where *models.Business) (*models.Business, error) {
	...
	query := fmt.Sprintf("SELECT business.id, business.name, business_category.name as business_category, business.business_brand_id, business.municipality_id, business.province_id, business.thumbnail, business.blurhash, business.address, business.high_quality_photo, business.time_margin_order_month, business.time_margin_order_day, business.time_margin_order_hour, business.time_margin_order_minute, business.low_quality_photo, business.delivery_price, business.home_delivery, business.to_pick_up, business.cursor, ST_AsEWKB(business.coordinates) AS coordinates, %v FROM business INNER JOIN business_category ON business.business_category_id=business_category.id WHERE business.id = '%v' ORDER BY business.cursor asc LIMIT 1;", distance, where.ID)
	err := tx.Raw(query).Scan(&businessResult).Error
	if err != nil {
		return nil, err
